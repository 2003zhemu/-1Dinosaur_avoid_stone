...
local function each(t)
  ...
  if typeof(t) == "string" then
    getEnumerator = System.String.GetEnumerator
  else
    getEnumerator = t.GetEnumerator
  end
...
-- 后面参数为泛型
-- 供 params函数构造使用
function System.toLuaTuple(array,...)
  local t
  if array~=nil then
     t =System.toLuaTable(array)
  else
     t={}
  end
...
local function get(t, index)
  if System.is(index,System.Range) then
    local start,end_ = index.getStart(),index.getEnd()
    if start==nil then
      start = 1
    else 
      start = start +1
    ...
    local result = System.selectRange(t,start,end_)
    return setmetatable(result, System.List(t.__genericT__))
  end
