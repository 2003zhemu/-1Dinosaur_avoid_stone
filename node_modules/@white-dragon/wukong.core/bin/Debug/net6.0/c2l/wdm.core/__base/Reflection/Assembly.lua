...
local function checkValue(value, valueClass)
  if value == nil then
    if valueClass.class == "S" then
      ...
      -- function 特殊处理
      if type(value) == "function" and valueClass.__name__=="System.Delegate" then
        return value
      end
...
local ParameterInfo = define("System.Reflection.ParameterInfo", {
  ...
  getHasDefaultValue=function(this)
    return false  -- 无法获取参数默认值
  end
})

local ConstructorInfo = define("System.Reflection.ConstructorInfo", {
  data = nil,
  GetParameters = function (this)
    local t={}
    local metadata = this.data
    local flags = metadata[2]
    local parameterCount = getMethodParameterCount(flags)
    if parameterCount ~= 0 then
      for i = 4, 3 + parameterCount do
        local cls = metadata[i]
        local ptype = typeof(cls)
        local info = setmetatable({ ParameterType = ptype,Name=ptype:getName()}, ParameterInfo)
        t[i-3] = info
      end
    end
    return arrayFromTable(t, ParameterInfo)
  end,
  IsDefined = function (this, attributeType)
    if attributeType == nil then throw(ArgumentNullException()) end
    local metadata = this.metadata
  ...
  op_Inequality = function(constructor,b)
    return constructor ~= b
  end,
  op_Equality = function(constructor,b)
    return constructor == b
  end,
  Invoke = function(this, ...)

    local createInstance = function(...)
      local item = setmetatable({}, this.cls)
      return item,this.data[3](item,...)
    end
    ...
    elseif n == 1 then
      ...
      if System.isArrayLike(args) then
        ...
        return createInstance(unpack(args, 1, n))
      end
    end
...
function Type.GetFields(this,flags)
  local t = {}
  local cls = this[1]
  local count = 1
  local isPublic = true
  ...
  repeat
    ...
    if metadata then
      ...
      if fields then
        for i = 1, #fields do
          ...
          if hasPublicFlag(field[2])==isPublic then
    ...
    if isPublic and  not metadata and cls["__name__"]	~= "System.Object" then
      for k, v in pairs(cls) do
        if k~="__assembly__" and k~="__name__" and k~="__metadata__" and k~="__index" and type(v) ~= "function" then
...
function Type.GetConstructors(this)
  local t = {}
  local cls = this[1]
  local count = 1
  local tier = 1
  repeat
    ...
    if metadata then
      ...
      if methods then
        for i = 1, #methods do
          local m = methods[i]
          if m[1] == ".ctor" then
            local ctor = setmetatable({ data = m ,cls = cls ,tier=tier }, ConstructorInfo)
            t[count] = ctor
            count = count + 1
            --kk 补充constructor
            if m[3] == nil then
    ...
    tier = tier + 1
  until cls == nil 
  return arrayFromTable(t, ConstructorInfo)
end
...
Assembly = define("System.Reflection.Assembly", {
  ...
  GetAssembly2= function(assembly)
    return setmetatable(assembly, Assembly)
  end,
    ...
    while true do
      ...
      local clsName = name:sub(i, j - 1)
			if name:find("%[") >0 then
				...
				local c=System.getClass(clsName)
				t[count] = typeof(c)
			end
...
define("System.Reflection.CustomAttributeExtensions", {
  GetCustomAttributes = System.Reflection.MethodInfo.GetCustomAttributes,
})
