...
local equals = function(x, y) 



  -- nan == nan (如果是number且自己不等于自己，则为nan)
  if typeof(x)=="number" and x~=x then
    if typeof(y)=="number" and y~=y then
      return true
    end
  end
...
local function getHashCode2(x)
  if typeof(x)=="table" then
    local f = x["GetHashCode"]
    if f==nil then 
      f = x["getHashCode"]
    end
    if f~=nil then 
...
-- 选取array的一部分range
-- start 默认为1, end_默认为最大值
local function selectRange(tab,start,end_)
  if start==nil then
    start = 1
  end
...
local RobloxIntMax = 4294967296
System.RobloxIntMax =RobloxIntMax

  local bnot, band, bor, bxor, sl, sr
  ...
  bnot = function(x)
    ...
    elseif x<0 then 
      ...
      return bit32.bnot(x) - RobloxIntMax
    end
  end
  band =  function(x,y)
  ...
  function System.toInt32(v, checked)
    ...
    if v > 2147483647 and v<=4294967296 then
      return v- 4294967296
    end 
...
function System.using(t, f)
  ...
  if dispose ~= nil then
    ...
    if not ok then
      ...
      -- 有时候status不能打印,补充message
      warn("error_message"..status.message)
      error(status)
    end
    return status, ret
  else
...
if debugsetmetatable then
  ...
  compareObj = function (a, b)
    ...
    elseif t == "string" then
      return System.String.CompareToObj(a, b)
    else
      local ia = a.CompareToObj
      if ia ~= nil then
        return ia(a, b)
...
System.toString2 = toString2

-- kk, 可空类型
System.nullableOp = function(x,y,operator)
  if x==nil or y==nil then
    return nil
  end
  ...
  elseif operator == ">=" then
    ...
    throw(System.NotSupportedException("nullable bin operator not supported"))
  end
end
...
defCls("System.SerializableAttribute", { base = { Attribute } })
...
function System.namespace(name, f)
  if not assembly then 
    assembly = setmetatable({}, namespace) 
  end
  namespace[1] = name
  f(namespace)
...
function System.init(t)
  ...
  if types then
    ...
    for i = 1, #types do
      local name = types[i]
      local cls = assert(modules[name], name.." not exist!")()
      classes[i] = cls
    end
  ...
  table.insert(System.assemblies,assembly)
  modules, imports, assembly, metadatas = {}, {}, nil, nil
  return current,t
end

-- kk:移除类型
function System.remove(className)
  local scope = global
    local starIndex = 1
    while true do
      local pos = sfind(className, "[%.+]", starIndex) or 0
...
-- kk:移除Assembly
function System.removeAssembly(assembly)
  local newAssemblies = {}
  for i = 1, #System.assemblies do
  
    if System.assemblies[i]~=assembly then
      table.insert(newAssemblies,System.assemblies[i])
    end
  end
  System.assemblies = newAssemblies
end
