...
--!
    ...
    local function setPointerByte(n)
        ...
        assert(n >= 0, "argument #1 to BitBuffer.setPointerByte should not be negative")
    ...
    local function writeByteWithoutAssert(n)
        if bits == 0 then
...
--This is how Roblox does it and it works, so it was chosen to do it this way too.
    ...
    local function readBit()
        assert(pointer + 1 <= bitCount, "BitBuffer.readBits cannot read past the end of the stream")
        ...
        local output = BOOL_TO_BIT[bit32.btest(byte, pow)]-- Test if a bit is on by & ing it by 2^[bit position]
        c = c + 1
        ...
        pointer = pointer + 1 -- Move the pointer forward
        return output
    end
...
local upVector = NORMAL_ID_VECTORS[id % 6]
    ...
    return {
        ...
        writeByteWithoutAssert = writeByteWithoutAssert,
        writeBit = writeBit,
        writeBits = writeBits,
        writeByte = writeByte,
