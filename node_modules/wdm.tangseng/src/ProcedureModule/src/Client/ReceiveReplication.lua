...
local eventNameReplication = require(script.Parent.Parent.EventNames).EventReplication
...
local function setupReplication(world, state)
    ...
    eventBus.ConnectS2C(function(eventName,entities)
        if eventName~= eventNameReplication then
            return
        end
