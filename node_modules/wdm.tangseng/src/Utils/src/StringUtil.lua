-- by: CatchMoon
-- 注意：本脚本为EnginePlus.StringF的副本, 不会维护 | by: CatchMoon
...
function this.Num2Str(num: number, precision: number?, hideZero: boolean?): string
...
-- 为字符串追加玩家信息(玩家名称作为前缀)
function this.AppendPlayerInfo(str: string, player: Player): string
	return '[' .. player.Name .. "] " .. str
end

-- 给要显示在UI上的number加单位
function this.AppendUnit(n: number, precision: number?, hideZero: boolean?)
	...
	local Units = {
		...
		"Ud", "Dd", "Td", "Qt", "Qd", "Sd", "St", "Od", "Nod", "V", "Cv",
	}
	
	for i = 22, 1, -1 do
		if n >= math.pow(10, 3 * i) then
			local res = this.Num2Str((n / math.pow(10, 3 * i)), precision, hideZero)
	...
	local res = this.Num2Str(n, precision, hideZero)
	if res == "1000" then
		return '1' .. Units[2]
	else
		return res .. Units[1]
